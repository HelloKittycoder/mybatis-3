...
public class PooledDataSource implements DataSource {
   ...
   * 那么这个数据源允许这个线程尝试重新获取一个新的连接，但是这个重新尝试的次数不应该超过poolMaximumIdleConnections与
   * poolMaximumLocalBadConnectionTolerance之和。
   */
  protected int poolMaximumLocalBadConnectionTolerance = 3;
  /**
   * 发送到数据库的侦测查询，用来检验连接是否正常工作并准备接收请求。
   */
  protected String poolPingQuery = "NO PING QUERY SET";
  /**
   * 是否启用侦测查询。若开启，需要设置poolPingQuery属性为一个可执行的SQL语句（最好是一个速度非常快的SQL语句）
   */
  protected boolean poolPingEnabled;
  /**
   * 配置poolPingQuery的频率。可以被设置为和数据库连接超时时间一样，来避免不必要的侦测，默认值：0（即所有连接每一时刻都
   * 被侦测 - 当然仅当poolPingEnabled为true时适用）
   */
  protected int poolPingConnectionsNotUsedFor;

  /**
   * 期望Connection的类型编码，通过{@link #assembleConnectionTypeCode(String, String, String)}计算
   */
    ...
    synchronized (state) {
      ...
      if (conn.isValid()) { // 有效
        ...
        if (state.idleConnections.size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
          ...
          // 这段没看懂，需要请教下
          // 设置原连接失效
          // 为什么这里要创建新的PooledConnection对象呢？避免使用方还在使用conn，
    ...
    while (conn == null) {
      synchronized (state) {
        ...
        // 无空闲连接
          ...
          // 激活的连接数小于poolMaximumActiveConnections（激活连接数未达到上限）
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            ...
            if (longestCheckoutTime > poolMaximumCheckoutTime) { // 检查到超时
              ...
              try {
                ...
                // 等待，直到超时，或pingConnection方法中归还连接时的唤醒
  ...
  public static Connection unwrapConnection(Connection conn) {
    ...
    if (Proxy.isProxyClass(conn.getClass())) {
      ...
      // 如果是PooledConnection对象，则获取真实的链接
  ...
  // forceCloseAll会被finalize方法所调用，即当前PooledDataSource对象被释放时
